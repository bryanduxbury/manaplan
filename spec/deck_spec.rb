...
describe Deck do
  ...
  context "when determining castability" do
    it "should return not castable when there isn't enough (converted)" do
      castable, remaining_land = Deck.cast(Card.new("Talrand's Invocation", "2UU"), [])
      castable.should == false
    end

    it "should return not castable when there isn't enough (colored)" do
      castable, remaining_land = Deck.cast(Card.new("Talrand's Invocation", "2UU"), [BasicLand.new("B")] * 4)
      castable.should == false
    end

    it "should tap all the mana when available is exactly the cost" do
      castable, remaining_land = Deck.cast(Card.new("Talrand's Invocation", "2UU"), [BasicLand.new("B"), BasicLand.new("U")] * 2)
      castable.should == true
      remaining_land.should == []
    end

    it "should tap the correct colored lands when there is excess available" do
      castable, remaining_land = Deck.cast(Card.new("Talrand's Invocation", "2UU"), [BasicLand.new("B")] * 3 + [BasicLand.new("U")] * 2)
      castable.should == true
